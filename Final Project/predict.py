    ...
    for n in range(0, nb_batch):
        ...
        if not args.store_activations:
            ...
            for idx, row in enumerate(out):
                row = list(row)
                row.append(y_trues[idx])
                with open('output_'+args.model+'.csv', 'a') as csvFile:
...
if __name__ == '__main__':
    ...
    class_list = sorted([name for name in os.listdir(args.path)])
    print(class_list)
    for category in class_list:
        print(category)
        predict(args.path+str(category)+"/")
